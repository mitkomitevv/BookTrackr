...
export default function CurrentlyReadingCard({ book }) {
    ...
    const progressPath = user && book._id
        ? `/data/readingProgress?where=bookId%3D%22${book._id}%22%20and%20_ownerId%3D%22${user._id}%22`
        : null;
    
    const { data: progressData, refetch: refetchProgress } = useFetch(progressPath, {
    ...
    return (
        <>
            ...
            <Link to={`/catalog/${book._id}/details`} className="w-16 h-24 rounded-lg bg-slate-800 overflow-hidden flex-shrink-0">
                {book.coverUrl ? (
                    <img
                        src={book.coverUrl}
                        alt={book.title}
                            ...
                            <h3 className="text-sm font-semibold text-slate-100 line-clamp-2 group-hover:text-emerald-300">
                                ...
                                {book.series && (
                                    <span className="text-sm text-slate-400 font-normal ml-2">
                                        ({book.series}{book.numberInSeries ? `, #${book.numberInSeries}` : ""})
                                    </span>
                                )}
                            </h3>
                        </Link>
            ...
            >
                ...
                >
                    ...
                    <form onSubmit={updateProgressHandler}>
                        <div className="mb-4">
                            <label htmlFor="pageInput" className="block text-sm font-medium text-slate-300 mb-2">
                                Current page
                            </label>
                            <input
                                id="pageInput"
