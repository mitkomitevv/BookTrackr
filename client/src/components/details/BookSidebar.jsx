            ...
            <div className="space-y-2 text-xs text-slate-400">
                {shelfInfo ? (
                    <p>
                        ...
                        <span className="text-slate-200">{shelfInfo.shelf}</span> on{" "}
                        <span className="text-slate-200">{new Date(shelfInfo.addedDate).toLocaleDateString()}</span>.
                    </p>
                ) : (
                    <p>
...
function SimilarBooksSection({ bookId }) {
    const { data: currentBook, loading: bookLoading } = useFetch(`/data/books/${bookId}`);
    ...
    const similarBooks = useMemo(() => {
        if (!currentBook || !Array.isArray(allBooks) || !Array.isArray(ratingsData)) {
            return [];
        }
        ...
        const scoredBooks = allBooks
            ...
            .map(book => {
                ...
                if (book.tags) {
                    const matchingTags = book.tags.filter(tag => currentTags.includes(tag));
        ...
        return scoredBooks.map(book => {
            const bookRatings = ratingsData.filter(r => r.bookId === book._id);
            const avgRating = bookRatings.length > 0
                ? (bookRatings.reduce((sum, r) => sum + r.stars, 0) / bookRatings.length).toFixed(2)
                : 0;
            return { ...book, averageRating: parseFloat(avgRating), ratingCount: bookRatings.length };
        });
    ...
    if (similarBooks.length === 0) {
        return (
            <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 space-y-3 text-sm">
                ...
                <div className="text-xs text-slate-400">No similar books found</div>
            </section>
        );
    }
                ...
                {similarBooks.map((book) => (
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-slate-100 text-xs truncate">{book.title}</p>
