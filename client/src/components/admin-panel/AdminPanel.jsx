...
export default function AdminPanel() {
    ...
    const [resolvedPicks, setResolvedPicks] = useState({ pick: null, staff: [] });
    ...
    useEffect(() => {
        ...
        const ids = Array.from(new Set([...(pickId ? [pickId] : []), ...staffIds]));
        ...
        (async () => {
            try {
                const inside = ids.map(id => id.toString().replace(/"/g, '\\"')).join('","');
                const clause = `_id in ("${inside}")`;
                const res = await request(`/data/books?where=${encodeURIComponent(clause)}`);
                if (!mounted) return;
                const map = new Map((Array.isArray(res) ? res : []).map(b => [b._id || b.id, b]));
    ...
    const saveSettings = async () => {
        ...
        try {
            ...
            setMessage({ type: "error", text: err?.payload?.message || err.message || "Failed to save." });
        } finally {
            setSaving(false);
        }
    };
        ...
        <main className="flex-1">
            <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
                <header className="space-y-2">
                    <h1 className="text-2xl sm:text-3xl font-semibold text-slate-50">Admin panel</h1>
                    <p className="text-sm text-slate-400">Manage home page picks and staff recommendations.</p>
                </header>

                <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-6 sm:p-8 shadow-lg space-y-6">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <h2 className="text-sm font-semibold text-slate-100 uppercase tracking-wide">Pick of the Month</h2>
                            <p className="text-xs text-slate-400">Select a single book to highlight on the home page and however much you want for staff recommendations.</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
                            {booksLoading && <div className="text-sm text-slate-400">Loading books…</div>}
                        ...
                        {message && (
                            <div className={`text-sm ${message.type === "error" ? "text-red-400" : "text-emerald-300"}`}>
                                {message.text}
                            </div>
                        )}
