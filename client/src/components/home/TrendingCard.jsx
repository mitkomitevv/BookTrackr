...
export default function TrendingCard() {
    ...
    const { data: ratingsData, loading: ratingsLoading } = useFetch('/data/ratings');
    ...
    const trendingBook = useMemo(() => {
        ...
        const weeklyRatings = ratingsData.filter(rating => rating._createdOn >= oneWeekAgo);

        const ratingCountByBook = {};
        weeklyRatings.forEach(rating => {
            ratingCountByBook[rating.bookId] = (ratingCountByBook[rating.bookId] || 0) + 1;
        });
        ...
        const bookDetails = booksData.find(book => book._id === bookWithMostRatings);
        
        if (!bookDetails) {
            return null;
        }

        const bookRatings = weeklyRatings.filter(rating => rating.bookId === bookWithMostRatings);
        const averageRating = bookRatings.length > 0
            ? (bookRatings.reduce((sum, rating) => sum + rating.stars, 0) / bookRatings.length).toFixed(2)
            : 0;
    ...
    const { shelves, bookShelves, toggleShelf, removeFromAllShelves } = useShelfManagement(trendingBook?._id, user);
    ...
    return (
        <Link to={`/catalog/${trendingBook._id}/details`} className="block w-full md:w-80 rounded-2xl border border-slate-800 bg-slate-900/60 p-4 space-y-3 backdrop-blur hover:border-slate-700 transition-colors">
            <p className="text-xs font-medium text-emerald-300 uppercase">
                Trending this week
            </p>
            <div className="flex gap-3">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-400">{trendingBook.author}</p>
            ...
            <div className="flex items-center justify-between text-xs pt-2">
                <div className="flex items-center gap-1 text-amber-300">
                    ...
                    <span className="text-slate-400">· {trendingBook.ratingCount.toLocaleString()} ratings</span>
                </div>
                <button 
                    onClick={(e) => {
                        e.preventDefault();
