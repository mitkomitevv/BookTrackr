...
export function useShelfManagement(bookId, user) {
    ...
    const { data: shelvesData, refetch: refetchShelves } = useFetch(shelvesPath, {
    ...
    useEffect(() => {
        const createInitialShelves = async () => {
            if (user && shelvesData && Array.isArray(shelvesData) && shelvesData.length === 0 && !isCreatingShelves) {
                setIsCreatingShelves(true);
                try {
                    const headers = { "X-Authorization": user.accessToken };
                    await request("/data/shelves", "POST", {
                        read: [],
                        currentlyReading: [],
                        "to-read": [],
                        favoriteBooks: [],
            ...
            if (!bookInShelf) {
                ...
                if (isExclusiveShelf) {
                    exclusiveShelfGroup.forEach(key => {
                        if (key !== shelfKey && updatedShelves[key]?.includes(bookId)) {
                            updatedShelves[key] = updatedShelves[key].filter(id => id !== bookId);
                        }
                    });
                }
            } else {
                updatedShelves[shelfKey] = currentShelf.filter(id => id !== bookId);
            }
    ...
    const removeFromAllShelves = async () => {
        ...
        try {
            ...
            const shelfKeys = ['currentlyReading', 'to-read', 'read', 'favoriteBooks', 'dnf'];
