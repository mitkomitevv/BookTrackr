...
export default function MyLibrary() {
    ...
    const shelvesPath = user ? `/data/shelves?where=_ownerId%3D%22${user._id}%22` : null;
    const { data: shelvesData, loading: shelvesLoading } = useFetch(shelvesPath, {
    ...
    const { data: currentlyReadingData, loading: crLoading } = useFetch(currentlyReadingPath, {
        headers: user?.accessToken ? { "X-Authorization": user.accessToken } : {},
    });
    ...
    const pillClasses = (enabled) => enabled
            ...
            <div className="max-w-6xl mx-auto px-4 py-8 space-y-10">
                ...
                <section className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    <div className="grid grid-cols-3 gap-3 text-xs sm:text-sm">
                        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 px-3 py-2.5 flex flex-col">
                            ...
                            <span className="text-lg font-semibold text-slate-50">{totalBooks}</span>
                        </div>
                        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 px-3 py-2.5 flex flex-col">
                            <span className="text-slate-400">Currently reading</span>
                            <span className="text-lg font-semibold text-emerald-400">{shelves?.currentlyReading?.length || 0}</span>
                        </div>
                        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 px-3 py-2.5 flex flex-col">
                            <span className="text-slate-400">Read this year</span>
                            <span className="text-lg font-semibold text-slate-50">{shelves?.read?.length || 0}</span>
                        </div>
                    </div>
                </section>
                ...
                <section className="space-y-4">
                    ...
                    {currentlyReadingBooks.length > 0 ? (
                        ...
                        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-8 text-center space-y-3">
                            <p className="text-slate-300">You're not reading anything right now.</p>
                            <p className="text-sm text-slate-400">Browse the catalog and start your next adventure!</p>
                ...
                {!hasAnyBooks && (
                    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-8 text-center space-y-4">
                        <h3 className="text-xl font-semibold text-slate-50">Your library is empty</h3>
                        <p className="text-slate-400 max-w-md mx-auto">
                            Start building your personal library by adding books to your shelves. 
                            Track what you're reading, plan your next reads, and keep a record of everything you've finished.
                        </p>
                        <Link to="/catalog" className="inline-flex items-center rounded-2xl bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-950 shadow hover:bg-emerald-400 transition">
                            Explore Books
                        </Link>
                ...
                {shelves && hasAnyBooks && (
                    <>
                        ...
                        {shelves.favoriteBooks && shelves.favoriteBooks.length > 0 && (
                            <ShelfPreview
                                title="Favorites"
