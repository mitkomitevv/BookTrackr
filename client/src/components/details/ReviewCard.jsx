...
export default function ReviewCard({
    ...
    const fetchLikesData = useCallback(async () => {
        try {
            ...
            const response = await request(`/data/reviewLikes?where=${whereClause}`, "GET", undefined, headers);
            ...
            if (Array.isArray(response)) {
                ...
                if (user?.accessToken) {
                    const userLike = response.find(like => like._ownerId === user._id);
                    if (userLike) {
                        setUserHelpful(true);
    ...
    const helpfulUpdateHandler = async () => {
        ...
        try {
            ...
            if (userHelpful && likeId) {
                ...
                await request(`/data/reviewLikes/${likeId}`, "DELETE", undefined, headers);
                setHelpfulCount(helpfulCount - 1);
                setUserHelpful(false);
                setLikeId(null);
            } else {
                // Like - create a new like record
                const likeResponse = await request(`/data/reviewLikes`, "POST", { reviewId: review.id }, headers);
                setHelpfulCount(helpfulCount + 1);
                setUserHelpful(true);
        ...
        <article className={`rounded-2xl border p-4 space-y-3 text-sm ${highlighted ? "border-emerald-500 bg-emerald-950/30 ring-1 ring-emerald-500/50" : "border-slate-800 bg-slate-900/70"}`}>
            ...
            <div className="flex items-center gap-3 text-[13px] text-slate-400">
                <button
                    ...
                    className={`inline-flex items-center gap-1 transition ${userHelpful
                            ? "text-emerald-400 hover:text-slate-400"
                            : "hover:text-emerald-400 disabled:text-slate-600 disabled:cursor-not-allowed"
                        }`}
                >
                    <svg xmlns="http://www.w3.org/2000/svg" fill={userHelpful ? "currentColor" : "none"} viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
